...
class DDPGbase(object):
    ...
    def Q_value(self, s, a):
        return self.sess.run(
            self.Qvalue, {self.S: s[np.newaxis, :], self.a: a[np.newaxis, :]}
        )[0]
...
class DDPGlearning:
    ...
    def learn_from_mix(
        self, model, initial_state, state_observe, state_update, op_profile, op_strategy
    ):
        """
            ...
            for j in range(MAX_EP_STEPS):
                ...
                if np.random.random() >= epsilon:
                    action = self.ddpg.choose_action(
                        state
                    )  # action has been normalized by choos_action()
                else:
                    if self.mode == "defend":
                        if EXPLORATION == "discrete":
                            ...
                            action = normalized(
                                [np.random.random() for k in range(self.action_size)]
                            )
                    else:
                        if EXPLORATION == "discrete":
                            action = [
                                np.random.randint(0, 5) * 0.25
                                for k in range(self.action_size)
                            ]
                        elif EXPLORATION == "continuous":
                            action = [
                ...
                (next_global_state, loss) = state_update(
                    TRAINING_MODE, global_state, list(action), op_action
                )
                next_state = np.array(
                    state_observe(next_global_state), dtype=np.float32
                )
                reward = -1.0 * loss
                ...
                if self.ddpg.pointer > MEMORY_INIT and (j + 1) % LEARNING_STEP == 0:
                    ...
                    if j == MAX_EP_STEPS - 1:
                        if (i + 1) % PRINT_STEP == 0:
                            logging.info(
                                "Episode {}, Ave step reward {}".format(
                                    i + 1, episode_reward / MAX_EP_STEPS
                                )
                            )
    ...
    def evaluate(
        ...
        for i in range(TEST_EPISODES):
            ...
            for j in range(MAX_TEST_STEPS):
                ...
                (next_global_state, loss) = state_update(
                    TEST_MODE, global_state, list(action), op_action
                )
                next_state = np.array(
                    state_observe(next_global_state), dtype=np.float32
                )
        ...
        for i in range(TEST_EPISODES_NS):
            ...
            for j in range(MAX_TEST_STEPS):
                if self.mode == "defend":
                    action = np.array(
                        normalized(test_defense_newest(model, global_state)[0]),
                        dtype=np.float32,
                    )
                else:
                    action = np.array(
                        test_attack_action(model, state), dtype=np.float32
                    )
                (next_global_state, loss) = state_update(
                    TEST_MODE, global_state, list(action), op_action
                )
    ...
    def policy(self, model: Model, state: Model.State):
        """
        ...
        if self.mode == "defend":
            ...
            delta = model.make_investigation_feasible(
                state.N, unflatten_list(action, len(model.alert_types))
            )
...
class DefenderOracle:
    ...
    def __init__(
        ...
        """
        self.mode = "defend"
        self.agent = DDPGlearning(
            self.mode,
            len(model.alert_types) * model.horizon,
            len(model.alert_types) * model.horizon,
        )
        ...
        def state_update(mode, state, action, alpha):
            ...
            delta = model.make_investigation_feasible(
                state.N, unflatten_list(action, len(model.alert_types))
            )  # make_investigation_feasible ``unnormalizes'' the action
            next_state = model.next_state(mode, state, delta, alpha)
            loss = next_state.U - state.U
            return (next_state, loss)

        if not os.path.exists(
            "../model/defender-{}-{}-{}/ddpg.ckpt.meta".format(
                exper_index, iteration_index, trial_index
            )
        ):
            print("defend-{}-{}_{}".format(exper_index, iteration_index, trial_index))
            cmd = "mkdir ../model/defender-{}-{}-{}".format(
                exper_index, iteration_index, trial_index
            )
            os.system(cmd)
            self.agent.learn_from_mix(
                model,
                Model.State(model),
            ...
            saver.save(
                self.agent.ddpg.sess,
                "../model/defender-{}-{}-{}/ddpg.ckpt".format(
                    exper_index, iteration_index, trial_index
                ),
            )
        # else:
...
class AttackerOracle:
    ...
    def __init__(
        ...
        """
        self.mode = "attack"
        state_size = model.horizon * (
            len(model.alert_types)
            + len(model.attack_types)
            + len(model.alert_types) * len(model.attack_types)
        )
        ...
        if not os.path.exists(
            "../model/attacker-{}-{}-{}/ddpg.ckpt.meta".format(
                exper_index, iteration_index, trial_index
            )
        ):
            print("attack-{}-{}_{}".format(exper_index, iteration_index, trial_index))
            cmd = "mkdir ../model/attacker-{}-{}-{}".format(
                exper_index, iteration_index, trial_index
            )
            os.system(cmd)
            self.agent.learn_from_mix(
            ...
            saver.save(
                self.agent.ddpg.sess,
                "../model/attacker-{}-{}-{}/ddpg.ckpt".format(
                    exper_index, iteration_index, trial_index
                ),
            )
        self.agent.evaluate(
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 7:
        print(len(sys.argv))
        print(
            "python ddpg.py [dataset] [mode] [def_budget] [adv_budget] [n_experiment] [opponent's policy]"
        )
        sys.exit(1)
    ...
    def evaluation(exper_index):
        ...
        if mode == "defend":
            if opponent == "uniform":
                oracle = DefenderOracle(
                    model, [test_attack_action], [1.0], exper_index, 0, 0
                )
            elif opponent == "aics":
                oracle = DefenderOracle(
                    model, [test_attack_aics], [1.0], exper_index, 0, 0
                )
        elif mode == "attack":
            if opponent == "uniform":
                oracle = AttackerOracle(
                    model, [test_defense_newest], [1.0], exper_index, 0, 0
                )
            elif opponent == "aics":
                oracle = AttackerOracle(
                    model, [test_defense_aics], [1.0], exper_index, 0, 0
                )
            elif opponent == "icde":
                oracle = AttackerOracle(
                    model, [test_defense_icde], [1.0], exper_index, 0, 0
                )
            elif opponent == "proportion":
                oracle = AttackerOracle(
                    model, [test_defense_proportion], [1.0], exper_index, 0, 0
                )
