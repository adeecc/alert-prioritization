...
def normalized(vect):
    ...
    vect = [element + EPS for element in vect]
    factor = 1 / sum(vect)
...
def flatten_state(state):
    ...
    R_list = flatten_lists(flatten_lists(state.R)) # No need to call it twice now, since numpy array
    state_list = np.hstack((N_list, M_list, R_list))
    return state_list
